-- ========================================
-- FIX: Send notification to both author AND clerk when completed
-- ========================================
...
BEGIN
  ...
  IF NEW.status = 'completed' AND (OLD.status IS NULL OR OLD.status != 'completed') THEN
    ...
    EXCEPTION
      WHEN undefined_table THEN
        RAISE LOG '⚠️  profiles table does not exist, skipping clerk notification';
      WHEN undefined_column THEN
        RAISE LOG '⚠️  Required columns do not exist, skipping clerk notification';
    END;
    ...
    IF clerk_chat_id IS NOT NULL AND clerk_chat_id != '' THEN
      ...
      RAISE LOG '📤 Sending COMPLETED notification to clerk: % (chat_id: %)', clerk_name, clerk_chat_id;
      PERFORM send_telegram_notification(payload);
    ELSE
...
SELECT 'Trigger function updated to notify both author and clerk when completed!' as status;
