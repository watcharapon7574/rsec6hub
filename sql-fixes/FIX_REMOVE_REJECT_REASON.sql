-- ========================================
-- FIX: Remove reject_reason field (use rejected_name_comment instead)
-- ========================================
...
BEGIN
  ...
  IF NEW.status = 'rejected' AND (OLD.status IS NULL OR OLD.status != 'rejected') THEN
    -- Get reject reason from rejected_name_comment JSONB
    IF NEW.rejected_name_comment IS NOT NULL THEN
      BEGIN
        reject_comment := NEW.rejected_name_comment::jsonb->>'comment';
      EXCEPTION
        WHEN OTHERS THEN
...
SELECT 'Trigger function has been fixed to use rejected_name_comment instead of reject_reason!' as status;
