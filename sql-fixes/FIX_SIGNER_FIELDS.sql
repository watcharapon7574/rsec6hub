-- ========================================
-- FIX: Correct field names in signer_list_progress
-- ========================================
...
BEGIN
  ...
  IF NEW.status = 'pending_sign' AND (OLD.status IS NULL OR OLD.status != 'pending_sign') THEN
    IF NEW.signer_list_progress IS NOT NULL THEN
      -- FIX: Use correct field names: 'name' and 'position' instead of 'signer_name' and 'signer_position'
      SELECT
        signer->>'user_id',
        signer->>'name',
        signer->>'position'
...
SELECT 'Trigger function has been fixed with correct field names!' as status;
