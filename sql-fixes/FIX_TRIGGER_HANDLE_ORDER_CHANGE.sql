-- ========================================
-- FIX: Handle notification when current_signer_order changes
-- ========================================
...
BEGIN
  ...
  -- FIX: Changed condition to handle both cases
  IF NEW.status = 'pending_sign' AND (
    (OLD.status IS NULL OR OLD.status != 'pending_sign') OR  -- Status changed to pending_sign
    (OLD.current_signer_order IS DISTINCT FROM NEW.current_signer_order)  -- Signer order changed
  ) THEN
...
SELECT 'Trigger function has been updated to handle signer order changes!' as status;
