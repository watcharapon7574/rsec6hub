                ...
                <div className="flex items-center gap-1 sm:gap-2 ml-2 flex-1 overflow-x-auto">
                  ...
                  {memo.status === 'draft' ? (
                    ...
                    <>
                      ...
                      <div className={`w-4 sm:w-5 h-0.5 mx-0.5 sm:mx-1 ${memo.status === 'completed' ? 'bg-muted' : 'bg-purple-200 dark:bg-purple-800'}`} />
                      ...
                      {memo.signer_list_progress && Array.isArray(memo.signer_list_progress) && memo.signer_list_progress.length > 0 ? (
                        ...
                        Array.isArray(memo.signature_positions) && memo.signature_positions.length > 0 ? (
                          ...
                          <span className={`text-[9px] ${memo.status === 'completed' ? 'text-muted-foreground' : 'text-purple-400 dark:text-purple-600'}`}>ไม่พบข้อมูลลำดับผู้ลงนาม</span>
                        )
                      )}
