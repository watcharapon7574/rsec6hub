...
BEGIN
  ...
  -- Note: These should be set as database settings or environment variables
  -- For now, we'll use pg_net extension to make HTTP calls

  -- Make async HTTP request using pg_net (non-blocking)
  SELECT net.http_post(
    url := current_setting('app.supabase_url', true) || '/functions/v1/telegram-notify',
...
BEGIN
  ...
  -- Get author's telegram_chat_id from employees table
