-- Migration: Add notification for clerks when new memo is created (draft status)
-- Description: Send Telegram notification to all clerks when a new memo is created

-- =====================================================
-- Update the main notification trigger function
-- =====================================================
...
-- =====================================================
-- Create INSERT triggers for new documents
-- =====================================================

-- Trigger for INSERT on memos table (when new memo is created)
DROP TRIGGER IF EXISTS telegram_notify_on_memo_insert ON memos;
CREATE TRIGGER telegram_notify_on_memo_insert
  AFTER INSERT ON memos
  FOR EACH ROW
  WHEN (
    -- Only trigger when status is draft
    NEW.status = 'draft'
    AND NEW.doc_del IS NULL  -- Skip soft-deleted documents
  )
  EXECUTE FUNCTION notify_telegram_on_memo_status_change();

-- Trigger for INSERT on doc_receive table (when new doc_receive is created)
DROP TRIGGER IF EXISTS telegram_notify_on_doc_receive_insert ON doc_receive;
CREATE TRIGGER telegram_notify_on_doc_receive_insert
...
-- =====================================================
COMMENT ON FUNCTION notify_telegram_on_memo_status_change() IS 'Trigger function to send Telegram notifications when document status changes (draft/created, rejected, pending_sign, completed)';
COMMENT ON TRIGGER telegram_notify_on_memo_insert ON memos IS 'Automatically send Telegram notifications to clerks when new memo is created';
COMMENT ON TRIGGER telegram_notify_on_doc_receive_insert ON doc_receive IS 'Automatically send Telegram notifications to clerks when new doc_receive is created';
