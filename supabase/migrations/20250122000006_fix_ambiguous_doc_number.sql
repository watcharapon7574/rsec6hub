-- Migration: Fix ambiguous column reference in get_user_assigned_tasks
-- Description: แก้ไขปัญหา column reference "doc_number" is ambiguous

-- =====================================================
-- Fix get_user_assigned_tasks function
-- =====================================================
...
BEGIN
  ...
  SELECT
    ...
    CASE
      WHEN ta.document_type = 'memo' THEN m.doc_number
      WHEN ta.document_type = 'doc_receive' THEN dr.doc_number
      ELSE NULL
...
COMMENT ON FUNCTION get_user_assigned_tasks(UUID, VARCHAR, INT, INT) IS 'ดึงรายการงานที่ได้รับมอบหมายของผู้ใช้ (Fixed ambiguous column reference)';
