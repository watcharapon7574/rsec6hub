-- Fix telegram notification trigger to handle memos table correctly
-- The memos table doesn't have user_id field, it has created_by instead
...
BEGIN
  ...
  -- Get the user_id based on table type
  -- memos table uses 'created_by', doc_receive uses 'user_id'
  IF is_doc_receive THEN
    memo_user_id := NEW.user_id;
  ELSE
    -- memos table uses created_by field
    memo_user_id := NEW.created_by;
  END IF;
