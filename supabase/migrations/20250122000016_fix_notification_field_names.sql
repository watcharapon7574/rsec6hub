-- Fix notification trigger to use correct field names from signer_list_progress
-- The actual fields are 'name' and 'position', not 'signer_name' and 'signer_position'
...
BEGIN
  ...
  IF NEW.status = 'pending_sign' AND (OLD.status IS NULL OR OLD.status != 'pending_sign') THEN
    ...
    IF NEW.signer_list_progress IS NOT NULL THEN
      ...
      -- Use correct field names: 'name' and 'position', not 'signer_name' and 'signer_position'
      SELECT
        signer->>'user_id',
        signer->>'name',
        signer->>'position'
