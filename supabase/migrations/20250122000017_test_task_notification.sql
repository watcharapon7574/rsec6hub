-- Test: Verify task assignment notification trigger is working
-- This migration adds detailed logging to the trigger
...
BEGIN
  ...
  IF NEW.document_type = 'memo' THEN
    ...
    RAISE LOG '🔔 [TASK NOTIFICATION] Memo details - subject: %, doc_number: %', doc_subject, doc_number;
  ELSIF NEW.document_type = 'doc_receive' THEN
    ...
    RAISE LOG '🔔 [TASK NOTIFICATION] Doc_receive details - subject: %, doc_number: %', doc_subject, doc_number;
  END IF;
...
COMMENT ON FUNCTION notify_on_task_assignment IS 'แจ้งเตือนผู้ที่ได้รับมอบหมายงานใหม่ (in-app + Telegram) - With detailed logging';
