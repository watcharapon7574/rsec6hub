-- แก้ไข function notify_telegram_on_memo_status_change ให้ส่งข้อมูลผู้ตีกลับไปด้วย
...
BEGIN
  ...
  IF NEW.status = 'rejected' AND (OLD.status IS NULL OR OLD.status != 'rejected') THEN
    -- Extract rejection comment and rejector info from rejected_name_comment JSONB
...
'Updated to include rejector name and position in document rejection notifications';
