-- Migration: Exclude report memos from assignment workflow
-- Description: ไม่ให้ report memo (รายงานผล) แสดงในหน้ามอบหมายงานและไม่แจ้งเตือนธุรการ
-- เมื่อ report memo อนุมัติครบแล้ว ให้จบ workflow เลย ไม่ต้อง loop กลับมามอบหมายใหม่
...
BEGIN
  RETURN QUERY
  -- Get completed memos (EXCLUDE report memos - subject starts with 'รายงานผล')
  SELECT
    m.id AS document_id,
  ...
  WHERE m.status = 'completed'
    ...
    -- EXCLUDE report memos (subject starts with 'รายงานผล')
    AND (m.subject IS NULL OR m.subject NOT LIKE 'รายงานผล%')
...
BEGIN
  ...
  -- AND is NOT a report memo (subject NOT starts with 'รายงานผล')
  IF NEW.current_signer_order = 5 AND
     ...
     NEW.doc_del IS NULL AND
     (NEW.subject IS NULL OR NEW.subject NOT LIKE 'รายงานผล%') THEN
...
-- =====================================================
-- PART 3: Recreate triggers with updated condition
-- =====================================================

-- Trigger for memos table - exclude report memos
...
CREATE TRIGGER trigger_notify_clerks_on_memo_completed
  ...
  WHEN (
    ...
    (NEW.is_assigned IS NULL OR NEW.is_assigned = false) AND
    NEW.doc_del IS NULL AND
    (NEW.subject IS NULL OR NEW.subject NOT LIKE 'รายงานผล%')
  )
  EXECUTE FUNCTION notify_clerks_on_document_completed();

-- Trigger for doc_receive table (no change needed - doc_receive can't be report memos)
...
-- =====================================================
COMMENT ON FUNCTION get_documents_ready_for_assignment IS 'ดึงรายการเอกสารที่พร้อมสำหรับการมอบหมาย - ไม่รวม report memo (รายงานผล)';
COMMENT ON FUNCTION notify_clerks_on_document_completed IS 'แจ้งเตือนธุรการเมื่อเอกสารเกษียนแล้ว - ไม่รวม report memo (รายงานผล)';
COMMENT ON TRIGGER trigger_notify_clerks_on_memo_completed ON memos IS 'แจ้งเตือนธุรการเมื่อบันทึกข้อความเกษียนแล้ว (ไม่รวม report memo)';
